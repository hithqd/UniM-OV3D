...
def all_gather_with_count(data):
    """
    Run all_gather on tensors
    Args:
        data: any tensor
    Returns:
        list[data]: size list of data gathered from each rank
        list[data]: list of data gathered from each rank
    """
    ...
    tensor_list = []
    for _ in size_list:
        tensor_list.append(torch.zeros(size=(max_size,), dtype=dtype).cuda())
    if local_size != max_size:
        padding = torch.zeros(size=(max_size - local_size,), dtype=dtype).cuda()
        tensor = torch.cat((tensor, padding), dim=0)
    dist.all_gather(tensor_list, tensor)
    ...
    return gathered_data, (np.array(size_list) / origin_size[1:].numel()).astype(np.int32).tolist()
